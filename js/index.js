...
async function loadInitialData(sClass) {
  ...
  try {
    ...
    let dayDisplay = days > 0 ? days + (days == 1 ? " day," : " day, ") : "";
...
function generateCountDown(ele, claimDate) {
  ...
  // update the countdown in every second
  countDownGlobal = setInterval(function () {
    //get today's date and time
    var now = new Date().getTime();

    //find the distanse between now and the countdown date
    var distanse = countdownDate - now;

    // time calculation for days, hours, minutes, seconds
    var days = Math.floor(distanse / (1000 * 60 * 60 * 24));
    var hours = math.floor(
      (distanse % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
    );
    ...
    // if the countdown is finised write some text
    if (distanse < 0) {
      clearInterval(countDownGlobal);
